...
CSoundManager::CSoundManager()
{
	...
	ZeroMemory( &m_dsListenerParams, sizeof( DS3DLISTENER ) );             // Listener properties
...
////////////////
    ...
    if( FAILED(hr) )
    {
        DXUT_ERR_MSGBOX( TEXT("Get3DListenerInterface"), hr );
		_ASSERT( 0 );
        return hr;
    }
...
HRESULT CSoundManager::CreateFromBuf( CSound** ppSound, char* buf, int bufsize, DWORD dwCreationFlags, GUID guid3DAlgorithm, DWORD dwNumBuffers )
{
    ...
    if( m_pDS == NULL )
        return CO_E_NOTINITIALIZED;
    if( ppSound == NULL || dwNumBuffers < 1 )
        return E_INVALIDARG;

    apDSBuffer = new LPDIRECTSOUNDBUFFER[dwNumBuffers];
    if( apDSBuffer == NULL )
    {
		DbgOut( "HuSound : SoundManager : CreateFromBuf : DSBuffer alloc : error !!!\n" );
        hr = E_OUTOFMEMORY;
        goto LFail;
    }
    ...
    if( pWaveFile == NULL )
    {
		DbgOut( "HuSound : SoundManager : CreateFromBuf : WaveFile alloc : error !!!\n" );
        hr = E_OUTOFMEMORY;
        goto LFail;
    }

    pWaveFile->OpenFromBuf( buf, bufsize, NULL, WAVEFILE_READ );

    //if( (pWaveFile->GetSize() == 0) || (pWaveFile->GetSize() > 1000000) )
	if( pWaveFile->GetSize() == 0 )
    {
        // Wave is blank, so don't create it.
		DbgOut( "HuSound : SoundManager : CreateFromBuf : wavefile size : error %d!!!\n", pWaveFile->GetSize() );
        hr = E_FAIL;
        goto LFail;
    }
////////////////////////
    WAVEFORMATEX* pwfx;
	pwfx = pWaveFile->GetFormat();
    if( pwfx == NULL )
    {
        //SetDlgItemText( hDlg, IDC_STATUS, TEXT("Invalid wave file format.") );
		DbgOut( "HuSound : SoundManager : CreateFromBuf : WAVEFORMATEX NULL : error !!!\n" );
        _ASSERT( 0 );
		hr = 1;
		goto LFail;
    }
	...
	if( pwfx->nChannels > 1 )
    {
		...
		DbgOut( "HuSound : SoundManager : CreateFromBuf : Wave file must be mono for 3D sound : too many channels : error !!!\n" );
		_ASSERT( 0 );
		hr = 1;
		goto LFail;
    }
    ...
    if( pwfx->wFormatTag != WAVE_FORMAT_PCM )
    {
        ...
        //SetDlgItemText( hDlg, IDC_FILENAME, TEXT("") );
		DbgOut( "HuSound : SoundManager : CreateFromBuf : wave file must be PCM for 3D sound : error !!!\n" );
		_ASSERT( 0 );
		hr = 1;
		goto LFail;
    }
...
////////////////////////
    ...
    if( FAILED(hr) )
    {
		...
		DbgOut( "HuSound : SoundManager : CreateFromBuf : CreateSoundBuffer : error !!!\n" );
    ...
    if( (dwCreationFlags & DSBCAPS_CTRLFX) == 0 )
    {
        for( i=1; i<dwNumBuffers; i++ )
        {
            if( FAILED( hr = m_pDS->DuplicateSoundBuffer( apDSBuffer[0], &apDSBuffer[i] ) ) )
            {
				...
				DbgOut( "HuSound : SoundManager : CreateFromBuf : DupulicateSoundBuffer : error !!!\n" );
                goto LFail;
            }
        }
    }
    ...
    *ppSound = new CSound( apDSBuffer, dwDSBufferSize, dwNumBuffers, pWaveFile, dwCreationFlags );
	...
	if( !(*ppSound) ){
		DbgOut( "HuSound : SoundManager : CreateFromBuf : csound alloc : error !!!\n" );
		hr = 1;
		goto LFail;
	}
...
///////////////
    ...
    if( FAILED( hr = (*ppSound)->Get3DBufferInterface( 0, &((*ppSound)->m_pDS3DBuffer )) ) )
    {
        DXUT_ERR_MSGBOX( TEXT("Get3DBufferInterface"), hr );
        //SetDlgItemText( hDlg, IDC_STATUS, TEXT("Could not get 3D buffer.") );
        //SetDlgItemText( hDlg, IDC_FILENAME, TEXT("") );
		DbgOut( "HuSound : SoundManager : CreateFromBuf : Get3DBuuferInterface : error !!!\n" );
		_ASSERT( 0 );
		goto LFail;
    }

    // Get the 3D buffer parameters
    (*ppSound)->m_dsBufferParams.dwSize = sizeof(DS3DBUFFER);
    (*ppSound)->m_pDS3DBuffer->GetAllParameters( &((*ppSound)->m_dsBufferParams) );

    // Set new 3D buffer parameters
    //(*ppSound)->m_dsBufferParams.dwMode = DS3DMODE_HEADRELATIVE;
	(*ppSound)->m_dsBufferParams.dwMode = DS3DMODE_NORMAL;
    (*ppSound)->m_pDS3DBuffer->SetAllParameters( &((*ppSound)->m_dsBufferParams), DS3D_IMMEDIATE );
                               ...
                               DWORD dwNumBuffers )
{
    ...
    if( apDSBuffer == NULL )
    {
		DbgOut( "HuSound : SoundManager : Create : DSBuffer alloc : error !!!\n" );
        hr = E_OUTOFMEMORY;
        goto LFail;
    }
    ...
    if( pWaveFile == NULL )
    {
		DbgOut( "HuSound : SoundManager : Create : WaveFile alloc : error !!!\n" );
        hr = E_OUTOFMEMORY;
        goto LFail;
    }
    ...
    {
        // Wave is blank, so don't create it.
		DbgOut( "HuSound : SoundManager : Create : wavefile size : error %s, %d!!!\n", strWaveFileName, pWaveFile->GetSize() );
        hr = E_FAIL;
        goto LFail;
    }
////////////////////////
    ...
    {
        //SetDlgItemText( hDlg, IDC_STATUS, TEXT("Invalid wave file format.") );
		DbgOut( "HuSound : SoundManager : Create : WAVEFORMATEX NULL : error !!!\n" );
        _ASSERT( 0 );
		hr = 1;
		goto LFail;
    }
	...
	if( pwfx->nChannels > 1 )
    {
        ...
        //SetDlgItemText( hDlg, IDC_STATUS, TEXT("Wave file must be mono for 3D control.") );
        //SetDlgItemText( hDlg, IDC_FILENAME, TEXT("") );
		DbgOut( "HuSound : SoundManager : Create : Wave file must be mono for 3D sound : too many channels : error !!!\n" );
		_ASSERT( 0 );
		hr = 1;
		goto LFail;
    }
    ...
    if( pwfx->wFormatTag != WAVE_FORMAT_PCM )
    {
        ...
        //SetDlgItemText( hDlg, IDC_STATUS, TEXT("Wave file must be PCM for 3D control.") );
        //SetDlgItemText( hDlg, IDC_FILENAME, TEXT("") );
		DbgOut( "HuSound : SoundManager : Create : wave file must be PCM for 3D sound : error !!!\n" );
		_ASSERT( 0 );
		hr = 1;
		goto LFail;
    }
...
////////////////////////
    ...
    if( FAILED(hr) )
    {
        ...
        // It might also fail if hardware buffer mixing was requested
        // on a device that doesn't support it.
		DbgOut( "HuSound : SoundManager : Create : CreateSoundBuffer : error !!!\n" );
        DXUT_ERR( "CreateSoundBuffer", hr );
    ...
    if( (dwCreationFlags & DSBCAPS_CTRLFX) == 0 )
    {
        for( i=1; i<dwNumBuffers; i++ )
        {
            ...
            {
                DXUT_ERR( "DuplicateSoundBuffer", hr );
				DbgOut( "HuSound : SoundManager : Create : DupulicateSoundBuffer : error !!!\n" );
                goto LFail;
            }
        }
    }
    ...
    *ppSound = new CSound( apDSBuffer, dwDSBufferSize, dwNumBuffers, pWaveFile, dwCreationFlags );
	...
	if( !(*ppSound) ){
		DbgOut( "HuSound : SoundManager : Create : csound alloc : error !!!\n" );
		hr = 1;
		goto LFail;
	}
...
///////////////
    ...
    if( FAILED( hr = (*ppSound)->Get3DBufferInterface( 0, &((*ppSound)->m_pDS3DBuffer )) ) )
    {
        ...
        //SetDlgItemText( hDlg, IDC_STATUS, TEXT("Could not get 3D buffer.") );
        //SetDlgItemText( hDlg, IDC_FILENAME, TEXT("") );
		DbgOut( "HuSound : SoundManager : Create : Get3DBuuferInterface : error !!!\n" );
		_ASSERT( 0 );
		goto LFail;
    }
...
int CSoundManager::Set3DListener( float fDopplerFactor, float fRolloffFactor )
{
...
int CSoundManager::Set3DListenerMovement( D3DXVECTOR3* pvPosition, D3DXVECTOR3* pvVelocity, D3DXVECTOR3* pvOrientFront, D3DXVECTOR3* pvOrientTop )
{
	DWORD dwApplyFlag = DS3D_IMMEDIATE;
                ...
                DWORD dwNumBuffers, CWaveFile* pWaveFile, DWORD dwCreationFlags )
{
	...
	m_pDS3DBuffer = NULL;	// 3D sound buffer
	ZeroMemory( &m_dsBufferParams, sizeof( DS3DBUFFER ) );
...
int CSound::SetRepeats( DWORD dwloop )
{
	m_dwloop = dwloop;
	return 0;
}
...
LPDIRECTSOUNDBUFFER CSound::GetFreeBuffer()
{
	...
	if( m_setstartpoint == 1 ){
		return m_apDSBuffer[m_freeindex];//!!!!!!!!!!!!!!!!!
	}
...
HRESULT CSound::SetStartPointOfSound( int src1000 )
{

    if( (m_apDSBuffer == NULL) || !m_pWaveFile )
        return CO_E_NOTINITIALIZED;
	...
	int dsbindex;
	dsbindex = GetFreeBufferIndex();
	LPDIRECTSOUNDBUFFER pDSB;
	pDSB = GetBuffer( dsbindex );
	if( !pDSB ){
		DbgOut( "csound : SetStartPointOfSound : GetBuffer error !!!\n" );
		return E_FAIL;
	}

	m_freeindex = dsbindex;
	m_setstartpoint = 1;

    WAVEFORMATEX* pwfx = m_pWaveFile->GetFormat();
    if( pwfx == NULL )
    {
		DbgOut( "CSound : SetStartPointOfSound : pwfx NULL error !!!\n" );
		return E_FAIL;
	}

	DWORD setpos;
	setpos = (pwfx->nAvgBytesPerSec * src1000) / 1000;

	hr = pDSB->SetCurrentPosition( setpos );
	if( FAILED( hr ) ){
		DbgOut( "csound : SetStartPointOfSound : SetCurrentPosition error !!!\n" );
		return hr;
	}
...
HRESULT CSound::Play3D( LPDS3DBUFFER p3DBuffer, DWORD dwPriority, DWORD dwFlags, LONG lFrequency )
{
    ...
    if( SUCCEEDED( hr ) )
    {
		...
		//hr = pDS3DBuffer->SetConeAngles( 360, 360, DS3D_IMMEDIATE );
...
int CSound::Set3DDistance( float fMinDistance, float fMaxDistance )
{
	DWORD dwApplyFlag = DS3D_IMMEDIATE;
...
//	}
	...
	if( m_apDSBuffer ){
		LPDIRECTSOUND3DBUFFER pDS3D;
		HRESULT hr;

		DWORD i;
		for( i=0; i<m_dwNumBuffers; i++ ){
			if( m_apDSBuffer[i] ){  
				hr = Get3DBufferInterface( i, &pDS3D );
				if( FAILED( hr ) ){
					DbgOut( "UTSound : Set3DDistance : Get3DBufferInterface error !!!\n" );
					_ASSERT( 0 );
					return 1;
				}
				pDS3D->SetAllParameters( &m_dsBufferParams, dwApplyFlag );
				pDS3D->Release();
			}
		}
	}
...
int CSound::Set3DMovement( D3DXVECTOR3* pvPos, D3DXVECTOR3* pvVel )
{
	D3DVECTOR vpos;
	D3DVECTOR vvel;
    ...
    memcpy( &m_dsBufferParams.vPosition, &vpos, sizeof(D3DVECTOR) );
    memcpy( &m_dsBufferParams.vVelocity, &vvel, sizeof(D3DVECTOR) );
...
***/
	...
	if( m_dwloop == 0 ){
		m_freeindex = GetFreeBufferIndex();
		m_setstartpoint = 1;//!!!!!!!!!!!!!!!!!!!!
		setindex = m_freeindex;
	}else{
		setindex = 0;
	}


	LPDIRECTSOUND3DBUFFER pDS3D = NULL;
	HRESULT hr;
	hr = Get3DBufferInterface( setindex, &pDS3D );
	if( FAILED( hr ) ){
		DbgOut( "UTSound : Set3DMovement : Get3DBufferInterface error !!!\n" );
		_ASSERT( 0 );
		return 1;
	}

	if( pDS3D ){
		pDS3D->SetAllParameters( &m_dsBufferParams, DS3D_IMMEDIATE );
		pDS3D->Release();
	}
...
int CSound::GetFrequency( DWORD* pfreq )
{
	if( !m_pWaveFile ){
		//DbgOut( "HuSound : Sound : GetFrequency : wavefile NULL error !!!\n" );
		_ASSERT( 0 );
		return 1;
	}
    ...
    if( pwfx == NULL )
    {
		//DbgOut( "HuSound : Sound : GetFrequency : pwfx NULL error !!!\n" );
		_ASSERT( 0 );
		return 1;
	}

	*pfreq = pwfx->nSamplesPerSec;
...
HRESULT CWaveFile::OpenFromBuf( char* buf, int bufsize, WAVEFORMATEX* pwfx, DWORD dwFlags )
{
   HRESULT hr;
    ...
    if( m_dwFlags == WAVEFILE_READ )
    {
		...
		MMIOINFO mmif;
		ZeroMemory( &mmif, sizeof( MMIOINFO ) );

		mmif.pchBuffer = buf;
		mmif.fccIOProc = FOURCC_MEM;
		mmif.cchBuffer = bufsize;


        m_hmmio = mmioOpen( NULL, &mmif, MMIO_ALLOCBUF | MMIO_READ );

        if( NULL == m_hmmio )
        {
			DbgOut( "wavefile : OpenFromBuf : mmioOpen error !!!\n" );
			_ASSERT( 0 );
			return -1;
        }
    ...
    else
    {
		...
		***/
		...
		DbgOut( "WaveFile : OpenFromBuf : dwFlags not supported error !!!\n" );
		_ASSERT( 0 );
		return -1;
    }
