...
int g_bumpEnable = 0;//ハードウェア的に可能かどうか
int g_bumpUse = 0;//実際に使うかどうか
...
//extern
extern int e3dRestoreDeviceObject( int resetflag );
extern int e3dInvalidateDeviceObject( int resetflag );

extern int g_activateapp;
extern int g_presentinterval;
...
//    m_dwMinStencilBits  = 8;
    ...
    m_pFontBig = new CD3DFont( _T("Arial"),  36, D3DFONT_BOLD );

    //m_pFont = new CD3DFont( _T("ＭＳ　Ｐゴシック"), 12, D3DFONT_BOLD );
    //m_pFontBig = new CD3DFont( _T("ＭＳ　Ｐゴシック"),  36, D3DFONT_BOLD );
...
// Desc:
//-----------------------------------------------------------------------------
HRESULT CD3DApplication::Create( HINSTANCE hInstance, HWND srchwnd, int fullscreenflag, int bits, int multisample, int gpuflag )
{
    HRESULT hr;

	m_MultiSampleType = (D3DMULTISAMPLE_TYPE)multisample;
    ...
    if( m_hWnd == NULL)
    {
		...
		m_dwWindowStyle = WS_POPUP|WS_CAPTION|WS_THICKFRAME|
	...
	***/
	...
	if( fullscreenflag != 0 ){
		int ret;
		width = m_rcWindowBounds.right - m_rcWindowBounds.left;
		height = m_rcWindowBounds.bottom - m_rcWindowBounds.top;
		ret = SetFullscreenInfo( fullscreenflag, width, height, bits );
		if( ret ){
			DbgOut( "d3dapp : Create : SetFullScreenInfo : cant fullscreen %d %d\n", width, height ); 
			_ASSERT( 0 );
		}
		DbgOut( "d3dapp : Create : SetFullScreenInfo : enable w %d, h %d\r\n", width, height ); 
	}
    ...
    if( FAILED( hr = Initialize3DEnvironment( gpuflag ) ) )
    {
        SAFE_RELEASE( m_pD3D );
        return DisplayErrorMsg( hr, MSGERR_APPMUSTEXIT );
    }
...
/***
        ...
        for( UINT iMode = 0; iMode < dwNumAdapterModes; iMode++ )
        {
            // Get the display mode attributes
            D3DDISPLAYMODE DisplayMode;
            hr = m_pD3D->EnumAdapterModes( iAdapter, iMode, &DisplayMode );
			if( hr != D3D_OK ){
				DbgOut( "app : BuildDeviceList : EnumAdapterModes error !!! %x\r\n", hr );
				_ASSERT( 0 );
			}
...
// Desc:
//-----------------------------------------------------------------------------
HRESULT CD3DApplication::Initialize3DEnvironment( int gpuflag )
{
    HRESULT hr;
	...
	//if( fullscreenflag == 1 ){
	//	m_bWindowed = FALSE;//!!!!!!!!!!!!!!!!!!!!!!!!!!!!! 強制的に、フルスクリーンにする。
...
//m_d3dpp.MultiSampleType        = D3DMULTISAMPLE_NONE;
	...
	if( m_MultiSampleType != 0 ){
		hr1 = m_pD3D->CheckDeviceMultiSampleType( m_dwAdapter,
			pDeviceInfo->DeviceType, m_d3dpp.BackBufferFormat, m_d3dpp.Windowed, m_MultiSampleType, &msQ1 );

		hr2 = m_pD3D->CheckDeviceMultiSampleType( m_dwAdapter,
			pDeviceInfo->DeviceType, m_d3dpp.AutoDepthStencilFormat, m_d3dpp.Windowed, m_MultiSampleType, &msQ2 );
	}

	if( (hr1 == D3D_OK) && (hr2 == D3D_OK) && (msQ1 != 0) && (msQ2 != 0) ){
		m_d3dpp.MultiSampleType = m_MultiSampleType;
		m_d3dpp.MultiSampleQuality = 0;
		DbgOut( "Initialize3DEnvironment : multisample %d, %d, %d\r\n", m_MultiSampleType, msQ1, msQ2 );
	}else{
		m_d3dpp.MultiSampleType = (D3DMULTISAMPLE_TYPE)0;
		m_d3dpp.MultiSampleQuality = 0;
		DbgOut( "Initialize3DEnvironment : multisample not use %d %d\r\n", msQ1, msQ2 );
...
m_d3dpp.PresentationInterval = g_presentinterval;
...
///////!!!!!!!!!!!!!!!!!!!!!!!!!!!!
		...
		int resetflag = 0;
        InvalidateDeviceObjects( resetflag );
        DeleteDeviceObjects();
        SAFE_RELEASE( m_pd3dDevice );
	}else{
		DbgOut( "d3dapp : Initialize3DEnvironment : CreateDevice error %x\r\n", hr );
...
/***				
        ...
        if( pAdapterInfo->devices[pAdapterInfo->dwCurrentDevice].DeviceType ==
            D3DDEVTYPE_REF )
        {
            hr = Initialize3DEnvironment( gpuflag );
        }
***/
    }
...
HRESULT CD3DApplication::ForceWindowed()
{
    ...
    if( FAILED( hr = Initialize3DEnvironment( 1 ) ) )
        return DisplayErrorMsg( hr, MSGERR_APPMUSTEXIT );
    m_bReady = TRUE;
...
int CD3DApplication::TestCooperativeAndReset()
{
	HRESULT hr;
...
DbgOut( "d3dapp : TestCooperativeAndReset : DEVICELOST !!!\n" );
			return 1;//!!!!!!!!!!!!!!!
		}
...
DbgOut( "d3dapp : TestCooperativeAndReset : ERR_DEVICENOTRESET !!!\n" );
...
int CD3DApplication::TestCooperative()
{
	HRESULT hr;
    ...
    if( FAILED( hr = m_pd3dDevice->TestCooperativeLevel() ) )
    {
        ...
        if( D3DERR_DEVICENOTRESET == hr )
        {
            // If we are windowed, read the desktop mode and use the same format for
            // the back buffer
			//if( (g_activateapp = 0) && (m_bReady == TRUE) ){
			if( m_bReady == TRUE ){
				if( m_bWindowed )
				{
					D3DAdapterInfo* pAdapterInfo = &m_Adapters[m_dwAdapter];
...
int CD3DApplication::GetD3DDevice( LPDIRECT3DDEVICE9* ppdev )
{
	*ppdev = m_pd3dDevice;
	return 0;
}
...
HRESULT CD3DApplication::InitDeviceObjects()
{
	...
	if( m_pFontBig )
    m_pFontBig->InitDeviceObjects( m_pd3dDevice );

	return S_OK;	
}

HRESULT CD3DApplication::RestoreDeviceObjects()
{
	if( m_pFont )
		m_pFont->RestoreDeviceObjects();
	
	if( m_pFontBig )
		m_pFontBig->RestoreDeviceObjects();

	int ret;
	int resetflag = 0;
	ret = e3dRestoreDeviceObject( resetflag );
	if( ret ){
		DbgOut( "d3dapp : e3dRestoreDeviceObject error !!!\n" );
		_ASSERT( 0 );
		return -1;
	}
...
HRESULT CD3DApplication::InvalidateDeviceObjects( int resetflag )
{
	if( m_pFont )
		m_pFont->InvalidateDeviceObjects();
	if( m_pFontBig )
		m_pFontBig->InvalidateDeviceObjects();

	int ret;
	ret = e3dInvalidateDeviceObject( resetflag );
	if( ret ){
		DbgOut( "d3dapp : e3dInvalidateDeviceObject error !!!\n" );
		_ASSERT( 0 );
		return -1;
	}
...
HRESULT CD3DApplication::DeleteDeviceObjects()
{
	...
	if( m_pFontBig )
		m_pFontBig->DeleteDeviceObjects();
...
int CD3DApplication::SetFullscreenInfo( int flag, DWORD width, DWORD height, int bits )
{

// width, height の決定。
	DWORD arw[6] = { 640, 800, 1024, 1280, 1400, 1600 };
	DWORD arh[6] = { 480, 600, 768, 1024, 1050, 1200 };
	int diffw;
	int mindiff = 100000;
	int minno = -1;
	DWORD newwidth;
	...
	if( flag == 1 ){
		...
		if( minno < 0 ){
			DbgOut( "d3dapp : SetFullScreenInfo : screen size error !!!\n" );
			_ASSERT( 0 );
			return 1;
		}
...
///////// width, height formatの合う候補をselectmode1に抽出。	
	//HRESULT hr;
	...
	D3DModeInfo* selectmode1[150];
	int selectnum1 = 0;

DbgOut( "d3dapp : SetFullScreenInfo : dwNumModes %d\n", pDeviceInfo->dwNumModes );

	int modeno;
	for( modeno = 0; modeno < (int)(pDeviceInfo->dwNumModes); modeno++ ){
		D3DModeInfo* curmode;
		curmode = &pDeviceInfo->modes[modeno];

		if( (curmode->Width == newwidth) && (curmode->Height == newheight) ){
			selectmode1[selectnum1] = curmode;
			selectnum1++;
		}
	}

	if( selectnum1 <= 0 ){
		DbgOut( "d3dapp : SetFullScreenInfo : cant find size or format error !!!\n" );
		_ASSERT( 0 );
		return 1;
	}

/////// format, DepthStencilFormatの合う候補をselectmode2に抽出。
	D3DModeInfo* selectmode2[150];
	int selectnum2 = 0;


DbgOut( "d3dapp : SetFullScreenInfo : selectnum1 %d\n", selectnum1 );

	for( modeno = 0; modeno < selectnum1; modeno++ ){
		D3DModeInfo* curmode;
		curmode = selectmode1[modeno];

		if( curmode->DepthStencilFormat == D3DFMT_D16 ){
			if( bits == 16 ){
				if( (curmode->Format == D3DFMT_X1R5G5B5) || (curmode->Format == D3DFMT_R5G6B5) || (curmode->Format == D3DFMT_A1R5G5B5) ){
					selectmode2[selectnum2] = curmode;
					selectnum2++;
				}
			}else{
	...
	if( selectnum2 <= 0 ){
		DbgOut( "d3dapp : SetFullScreenInfo : cant find Depth format error !!!\n" );
		_ASSERT( 0 );
		return 1;
	}

/////// ＨＡＬの使えるものを、selecemode3に抽出
	D3DModeInfo* selectmode3[150];
	int selectnum3 = 0;

DbgOut( "d3dapp : SetFullScreenInfo : selectnum2 %d\n", selectnum2 );

	for( modeno = 0; modeno < selectnum2; modeno++ ){
		D3DModeInfo* curmode;
		curmode = selectmode2[modeno];


		/***
		if( (curmode->dwBehavior & D3DCREATE_HARDWARE_VERTEXPROCESSING) || (curmode->dwBehavior & D3DCREATE_MIXED_VERTEXPROCESSING) ){
			selectmode3[selectnum3] = curmode;
			selectnum3++;
		}
		***/
	...
	if( selectnum3 <= 0 ){
		DbgOut( "d3dapp : SetFullScreenInfo : cant find HAL error !!!\n" );
		_ASSERT( 0 );
		return 1;
	}
	
////// 16bitの時は、D3DFMT_X1R5G5B5、D3DFMT_R5G6B5の順で、最初に見つかったものを採用。
////// 32bitの時は、D3DFMT_A8R8G8B8、D3DFMT_X8R8G8B8の順で、最初に見つかったものを採用 
	D3DModeInfo* finalmode;
	int isfind = 0;

DbgOut( "d3dapp : SetFullScreenInfo : selectnum3 %d\n", selectnum3 );

	for( modeno = 0; modeno < selectnum3; modeno++ ){
		D3DModeInfo* curmode;
		curmode = selectmode3[modeno];
		
		if( bits == 16 ){
			if( curmode->Format == D3DFMT_A1R5G5B5 ){
				finalmode = curmode;
				isfind = 1;
				break;
			}
		}else{
			if( curmode->Format == D3DFMT_A8R8G8B8 ){
				finalmode = curmode;
				isfind = 1;
				break;
			}
	...
	for( modeno = 0; modeno < selectnum3; modeno++ ){
		...
		if( bits == 16 ){
			if( curmode->Format == D3DFMT_X1R5G5B5 ){
				finalmode = curmode;
				isfind = 1;
				break;
			}
	...
	if( isfind == 0 ){
		for( modeno = 0; modeno < selectnum3; modeno++ ){
			...
			if( bits == 16 ){
				if( curmode->Format == D3DFMT_R5G6B5 ){
					finalmode = curmode;
					isfind = 1;
					break;
				}
			}else{
				if( curmode->Format == D3DFMT_X8R8G8B8 ){
					finalmode = curmode;
					isfind = 1;
					break;
				}
	...
	if( isfind == 0 ){
		DbgOut( "d3dapp : SetFullScreenInfo : format error !!!\n" );
		_ASSERT( 0 );
		return 1;
	}
...
////////
	...
	if( finalmodeno < 0 ){
		DbgOut( "d3dapp : SetFullScreenInfo : finalmodeno error !!!\n" );
		_ASSERT( 0 );
		return 1;
	}

	DbgOut( "d3dapp : SetFullScreenInfo : dwCurrentDevice %d, dwCuurentMode %d\r\n", pAdapterInfo->dwCurrentDevice, finalmodeno );
	DbgOut( "d3dapp : SetFullScreenInfo : width %d, height %d, foramat %d\n", finalmode->Width, finalmode->Height, finalmode->Format );


	pDeviceInfo->dwCurrentMode = finalmodeno;
...
int CD3DApplication::CheckFullScreenParams( int flag, SIZE chksize, int bits, int* flagptr, SIZE* validsize, int* bitsptr )
{
   HRESULT hr;
    ...
    m_pD3D = Direct3DCreate9( D3D_SDK_VERSION );
    if( m_pD3D == NULL ){
		DbgOut( "d3dapp : CheckFullScreenParams : pD3D create error !!!\n" );
		_ASSERT( 0 );
        return 1;
	}
    ...
    if( FAILED( hr = BuildDeviceList() ) )
    {
		DbgOut( "d3dapp : CheckFullScreenParams : BuildDeviceList error !!!\n" );
		_ASSERT( 0 );
        SAFE_RELEASE( m_pD3D );
        return 1;
    }
...
///////// for dbg
	{
		...
		for( adaptno = 0; adaptno < m_dwNumAdapters; adaptno++ ){
		    D3DAdapterInfo* pAdapterInfo = &m_Adapters[adaptno];

			for( devno = 0; devno < pAdapterInfo->dwNumDevices; devno++ ){

				D3DDeviceInfo*  pDeviceInfo  = &pAdapterInfo->devices[devno];
				...
				DbgOut( "adapt %d, device %d, devtype %s\r\n", adaptno, devno, strtype );

				//if( pDeviceInfo->DeviceType == D3DDEVTYPE_HAL ){
					
					for( modeno = 0; modeno < pDeviceInfo->dwNumModes; modeno++ ){

					    D3DModeInfo*    pModeInfo    = &pDeviceInfo->modes[modeno];

						
						switch( pModeInfo->Format ){
						case D3DFMT_X1R5G5B5:
							DbgOut( "adapter %d, device %d, mode %d, Width %d, Height %d, X1R5G5B5\r\n",
								adaptno, devno, modeno, pModeInfo->Width, pModeInfo->Height );
							break;
						case D3DFMT_R5G6B5:
							DbgOut( "adapter %d, device %d, mode %d, Width %d, Height %d, R5G6B5\r\n",
								adaptno, devno, modeno, pModeInfo->Width, pModeInfo->Height );
							break;
						case D3DFMT_A1R5G5B5:
							DbgOut( "adapter %d, device %d, mode %d, Width %d, Height %d, A1R5G5B5\r\n",
								adaptno, devno, modeno, pModeInfo->Width, pModeInfo->Height );
							break;
						case D3DFMT_X8R8G8B8:
							DbgOut( "adapter %d, device %d, mode %d, Width %d, Height %d, X8R8G8B8\r\n",
								adaptno, devno, modeno, pModeInfo->Width, pModeInfo->Height );
							break;
						case D3DFMT_A8R8G8B8:
							DbgOut( "adapter %d, device %d, mode %d, Width %d, Height %d, A8R8G8B8\r\n",
								adaptno, devno, modeno, pModeInfo->Width, pModeInfo->Height );
							break;
						default:
							DbgOut( "adapter %d, device %d, mode %d, Width %d, Height %d, OTHER %d\r\n",
								adaptno, devno, modeno, pModeInfo->Width, pModeInfo->Height, pModeInfo->Format );
							break;
						}
	...
	int ret;
	ret = SetFullscreenInfo( flag, (int)chksize.cx, (int)chksize.cy, bits );
	if( ret ){
		DbgOut( "d3dapp : CheckFullScreenParams : SetFullscreenInfo failed\n" );
		*flagptr = 0;
		Cleanup3DEnvironment();
		return 0;//!!!!!!!
	}
    ...
    D3DDeviceInfo*  pDeviceInfo  = &pAdapterInfo->devices[pAdapterInfo->dwCurrentDevice];
    D3DModeInfo*    pModeInfo    = &pDeviceInfo->modes[pDeviceInfo->dwCurrentMode];
	validsize->cx = pModeInfo->Width;
	validsize->cy = pModeInfo->Height;
...
int CD3DApplication::CheckFullScreenParams( int flag, DWORD w, DWORD h, int bits, int* flagptr, int* wptr, int* hptr, int* bitsptr )
{
...
///////// for dbg
	...
	int ret;
	ret = SetFullscreenInfo( flag, w, h, bits );
	if( ret ){
		DbgOut( "d3dapp : CheckFullScreenParams : SetFullscreenInfo failed\n" );
		*flagptr = 0;
		Cleanup3DEnvironment();
    ...
    D3DDeviceInfo*  pDeviceInfo  = &pAdapterInfo->devices[pAdapterInfo->dwCurrentDevice];
    D3DModeInfo*    pModeInfo    = &pDeviceInfo->modes[pDeviceInfo->dwCurrentMode];
	*wptr = (int)( pModeInfo->Width );
	*hptr = (int)( pModeInfo->Height );
...
int CD3DApplication::CheckRTFormat( int srcfmt, int* okflag )
{
	HRESULT hr;
    D3DAdapterInfo* pAdapterInfo = &m_Adapters[m_dwAdapter];
    D3DDeviceInfo*  pDeviceInfo  = &pAdapterInfo->devices[pAdapterInfo->dwCurrentDevice];
...
int CD3DApplication::CheckHDR( int* okflag, int* hdrfmt, int* lumifmt )
{
	*okflag = 0;//!!!!!!!!
	...
	hr = m_pD3D->CheckDeviceFormat( m_dwAdapter, 
		pDeviceInfo->DeviceType,
		pModeInfo->Format,
	...
	*hdrfmt = D3DFMT_A16B16G16R16F;
	...
	if( hr == D3D_OK ){
		*lumifmt = D3DFMT_R32F;
		...
		hr2 = m_pD3D->CheckDeviceFormat( m_dwAdapter, 
		pDeviceInfo->DeviceType,
		pModeInfo->Format,
		D3DUSAGE_RENDERTARGET,
		D3DRTYPE_TEXTURE,
...
int CD3DApplication::GetMaxMultiSampleNum( int bits, int iswindowmode, int* numptr )
{

//DbgOut( "check!!!, d3dapp GetMaxMultiSampleNum 0\r\n" );

    // Create the Direct3D object
    m_pD3D = Direct3DCreate9( D3D_SDK_VERSION );
    if( m_pD3D == NULL ){
		DbgOut( "d3dapp : CheckAntialias : pD3D create error !!!\n" );
		_ASSERT( 0 );
        return 1;
	}

//DbgOut( "check!!!, d3dapp GetMaxMultiSampleNum 1\r\n" );
	HRESULT hr;
	if( FAILED( hr = BuildDeviceList() ) ){
		_ASSERT( 0 );
		return 1;
	...
	if( bits == 16 ){
		ret = CheckMultiSample( iswindowmode, D3DFMT_X1R5G5B5, &num1 );
		_ASSERT( !ret );
		ret = CheckMultiSample( iswindowmode, D3DFMT_R5G6B5, &num2 );
		_ASSERT( !ret );
	}else{
		ret = CheckMultiSample( iswindowmode, D3DFMT_A8R8G8B8, &num1 );
		_ASSERT( !ret );
		ret = CheckMultiSample( iswindowmode, D3DFMT_X8R8G8B8, &num2 );
		_ASSERT( !ret );
	}

//DbgOut( "check!!!, d3dapp GetMaxMultiSampleNum 2\r\n" );

	*numptr = max( num1, num2 );
	
//DbgOut( "check!!!, d3dapp GetMaxMultiSampleNum 3\r\n" );
	Cleanup3DEnvironment();

//DbgOut( "check!!!, d3dapp GetMaxMultiSampleNum 4\r\n" );
	return 0;
}


int CD3DApplication::CheckMultiSample( int winmode, D3DFORMAT fmt, int* numptr )
{
	...
	*numptr = 0;
	...
	HRESULT hr1, hr2;
	int maxnum = 0;
	for( num = 16; num >= 2; num-- ){
		DWORD quality1 = 0;
		DWORD quality2 = 0;
		//hr = m_pD3D->CheckDeviceMultiSampleType( D3DADAPTER_DEFAULT, D3DDEVTYPE_HAL , fmt, winmode, (D3DMULTISAMPLE_TYPE)num, &quality );
		hr1 = m_pD3D->CheckDeviceMultiSampleType( m_dwAdapter, pDeviceInfo->DeviceType, fmt, winmode, (D3DMULTISAMPLE_TYPE)num, &quality1 );
		hr2 = m_pD3D->CheckDeviceMultiSampleType( m_dwAdapter, pDeviceInfo->DeviceType, pModeInfo->DepthStencilFormat, winmode, (D3DMULTISAMPLE_TYPE)num, &quality2 );

		if( (hr1 == D3D_OK) && (hr2 == D3D_OK) && (quality1 != 0) && (quality2 != 0) ){
			maxnum = num;
			break;
		}
	}
...
int CD3DApplication::CheckSpec( HINSTANCE hInstance, HWND srchwnd, int* okflag )
{
	*okflag = 0;//!!!!!!!!!!!!!!!!!!
    ...
    m_pD3D = Direct3DCreate9( D3D_SDK_VERSION );
    if( m_pD3D == NULL ){
		DbgOut( "d3dapp : CheckSpec : pD3D create error !!!\n" );
		_ASSERT( 0 );
        return 1;
	}
	HRESULT hr;
