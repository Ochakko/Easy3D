...
int CEQUFile::AddEQUMotion( int animno, CTreeHandler2* lpth, CShdHandler* lpsh, CMotHandler* lpmh, LONG* motidptr, float mvmult )
{
	*motidptr = -1;
	...
	m_motcookie = m_lpmh->AddMotion( ZA_1, curanimdat->animname, MOTION_CLAMP, curanimdat->frameleng, INTERPOLATION_SLERP, 0 );
	if( m_motcookie < 0 ){
	...
	*motidptr = m_motcookie;
	...
	for( bonecnt = 0; bonecnt < curanimdat->bonenum; bonecnt++ ){
		...
		if( boneseri > 0 ){
			...
			ret = m_lpsh->GetBonePos( boneseri, 0, -1, 0, &jpos, m_lpmh, inimat, 1 );
			_ASSERT( !ret );

			CShdElem* jse = (*m_lpsh)( boneseri );
			_ASSERT( jse );
			CShdElem* pse = m_lpsh->FindUpperJoint( jse );
			D3DXVECTOR3 ppos( 0.0f, 0.0f, 0.0f );

			if( pse ){
				ret = m_lpsh->GetBonePos( pse->serialno, 0, -1, 0, &ppos, m_lpmh, inimat, 1 );
				_ASSERT( !ret );
			}
...
//						srcmp.m_q.SetParams( 1.0f, 0.0f, 0.0f, 0.0f );
...
//						srcmp.m_q.w = curkeydat->q.w;
						...
						srcmp.m_mvx = mvmult * curkeydat->tra.x + ppos.x - jpos.x;
						srcmp.m_mvy = mvmult * curkeydat->tra.y + ppos.y - jpos.y;
						srcmp.m_mvz = mvmult * -curkeydat->tra.z + ppos.z - jpos.z;
