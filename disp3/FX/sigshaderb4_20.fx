...
//5: xyz=direction
	...
	rate2 = ( dot2 - mLightParams[lno][1].x ) * mLightParams[lno][1].y;
	rate2 = clamp( rate2, 0.0f, 1.0f );
...
//	finalmat = mWorldMatrixArray[0];
	...
	if( mLightNum >= 0.9f ){
		if( mLightParams[0][0].y == 0 ){
			...
			tmplight = CalcLightPoint( 0, v, tmppos, Normal.xyz, i.Power );
		}
	...
	if( mLightNum >= 1.9f ){
		if( mLightParams[1][0].y == 0 ){
			...
			tmplight = CalcLightPoint( 1, v, tmppos, Normal.xyz, i.Power );
		}
	...
	if( mLightNum >= 2.9f ){
		if( mLightParams[2][0].y == 0 ){
			...
			tmplight = CalcLightPoint( 2, v, tmppos, Normal.xyz, i.Power );
		}
...
//	finalmat = mWorldMatrixArray[0];
	...
	if( mLightParams[1][0].y == 0 ){
		...
		mag = dot( L2, L2 );
		L2 = normalize( L2 );
		Lrate2 = clamp( 1.0f - mag * mLightParams[1][0].z, 0.0f, 1.0f );
	}
	o.L2.xyz = L2;
	o.L2.w = Lrate2;
...
//	finalmat = mWorldMatrixArray[0];
	...
	if( mLightParams[2][0].y == 0 ){
		...
		mag = dot( L3, L3 );
		L3 = normalize( L3 );
		Lrate3 = clamp( 1.0f - mag * mLightParams[2][0].z, 0.0f, 1.0f );
	}
	o.L3.xyz = L3;
	o.L3.w = Lrate3;
...
VSPPSHADOW_OUTPUT1 SkinB0PPShadow1(VS_INPUT_TIMA i)
{
	...
	if( mLightParams[0][0].y == 0 ){
		...
		Lrate = 1.0f - dot( L, L ) * mLightParams[0][0].z;
		Lrate = clamp( Lrate, 0.0f, 1.0f );
	}
	o.L.xyz = L;
	o.L.w = Lrate;
...
PS_OUTPUT PSSkinPPL1( VSPPL1_OUTPUT In )
{
	...
	if( cmprate > 0.0 ){
		...
		mag = dot( L1, L1 );
		PSL1 = normalize( L1 );
		Lrate1 = clamp( 1.0f - mag * mLightParams[0][0].z, 0.0f, 1.0f );
	}
...
PS_OUTPUT PSSkinPPL3( VSPPL3_OUTPUT In )
{
	...
	float3 PSE = normalize( mEyePos.xyz - In.wPos.xyz );	
	float3 PSN = normalize(In.NandPow.xyz);
